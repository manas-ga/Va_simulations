...
## Function to add recalculated true V_A, lost_va to a dataframe

These calculations were performed at a later stage and are stored in a separate file ("../combined_data/new_va_calculation_combined.csv"). For every simulation in the input dataframem this functions looks up the row with the appropriate Set_ID in "new_va_calculation_combined.csv" and adds the V_A_true new and other data pertaining to lost_va to the inputted dataframe.
...
```{r}
d_new_va = read.csv("../combined_data/new_va_calculation_combined.csv")
...
Selection coefficients were drawn from a gamma distribution (shape = 0.3, scale = 0.033) and assigned to mutations randomly. The recombination rate in the msprime simulation was 5.32e-07. Map length in the burnin phase of just 1 generation (eng_gen = 2) was 5.
...
d_5 = add_new_va_data(d_new_va, d_5)
...
p_nb_2_std = make_std_plots(d_5_std, alpha = FALSE)$combined
p_nb_2_std
...
# Add calculations of vA_true_new, va_true_new, va_lost, va_lost_new
d_9 = add_new_va_data(d_new_va, d_9)
...
# Add calculations of vA_true_new, va_true_new, va_lost, va_lost_new
d_N1 = add_new_va_data(d_new_va, d_N1)
...
# Add calculations of vA_true_new, va_true_new, va_lost, va_lost_new
d_12 = add_new_va_data(d_new_va, d_12)
...
# Add calculations of vA_true_new, va_true_new, va_lost, va_lost_new
d_15 = add_new_va_data(d_new_va, d_15)
...
# Add calculations of vA_true_new, va_true_new, va_lost, va_lost_new
d_13 = add_new_va_data(d_new_va, d_13)
...
Plot the fraction of the initial additive genic variance lost by the end of the experiment as a function of non-neutral \eta_{scale}. Then, identify the simulations with the greatest and smallest losses and plot locuswise fraction V_a lost vs abs(\alpha) for these simulations. Finally, plot the error in our estimates of V_A as a function of the V_a lost in the simulations, colour-coded by \eta_{scale}.
...
plot_lost_va = ggplot(d_scale, aes(y = if(alpha){((va_true_new - va_left_new)/va_true_new)}else{((va_true - va_left)/va_true)}, x = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_boxplot() +
...
if(alpha){
  min_loss_sim = d_scale[which((d_scale$va_true_new - d_scale$va_left_new) == min(d_scale$va_true_new - d_scale$va_left_new)),]$Set_ID}else{
  min_loss_sim = d_scale[which((d_scale$va_true - d_scale$va_left) == min(d_scale$va_true - d_scale$va_left)),]$Set_ID  
  }
...
finescale_file_list = list.files(pattern = ".*finescale.*.csv$", full.names = TRUE, path = "../")
...
plot_finescale_va_left = ggplot(d_finescale_sel, aes(y = if(alpha){locuswise_va_left_new}else{locuswise_va_left},                                                      x = if(alpha){locuswise_va_true_new}else{locuswise_va_true}, 
                                                     color = lost_va_type)) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"($V_a$ left at a locus$)"), x = TeX(r"(Initial $V_a$ at a locus$)"), color = "Simulation type") +
  theme(text = element_text(size = 15)) +
  theme(legend.position = "bottom") +
  scale_color_manual(values = c("Min." = "#00BFC4", "Max." = "#F8766D"),
    labels = c("Min." = TeX(r"(Min. $V_a$ lost)"), "Max." = TeX(r"(Max. $V_a$ lost)"))  # Custom legend labels
