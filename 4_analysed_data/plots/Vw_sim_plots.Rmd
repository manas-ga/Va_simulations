...
```{r}
make_std_plots = function(data, font_size = 15, end_gen = 25000, data_in_title = FALSE){
...
p_nb_2_std = make_std_plots(d_5_std)$combined
p_nb_2_std
...
ggsave(p_nb_2_std, width = 8, height = 8, file = "Manuscript_figures/Fig1.jpg")
p_nb_2_std
...
p_nb_2_combined
ggsave(p_nb_2_combined, width = 12, height = 10, file = "Manuscript_figures/Fig2.jpg")
```
...
p_b_5_2_ml = ggplot(d_7_ml , aes(y=vA_est, x = vA_true, color = factor(sequence_length*r))) +
  ...
  geom_smooth(method = "lm", se=FALSE) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  labs(title = TeX(r"($\eta_{scale} = 0.1$)"))
...
p_b_large_std = make_std_plots(d_9_std)$combined
...
ggsave(plot = p_b_large_std, width = 8, height = 8, file = "Manuscript_figures/Fig3.jpg")
p_b_large_std
...
p_b_0.045_ml = ggplot(d_N1 , aes(y=vA_est, x = vA_true, color = factor(sequence_length*r))) +
  ...
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = "Map length \nin morgans\n(Experiment)", title = TeX(r"($\eta_{scale}$ = 0.045)")) + 
...
ggsave(p_b_0.045_ml, width = 6, height = 5, file = "manuscript_figures/FigS1.jpg")
...
```{r}
d_11 = read.csv("../combined_data/Set_11_output.csv") # Data for the set with mut_ratio = 0.0002
d_14 = read.csv("../combined_data/Set_14_output.csv") # Data for the set with mut_ratio = 0.02
d_9 = read.csv("../combined_data/Set_9_output.csv") # Data for the standard set
d_9_std = d_9[d_9$all.gp==FALSE&d_9$Ne_exp!="1000_1000",]
...
p_ben_0.033 = ggplot(rbind(d_9_std, d_11, d_14), aes(y = vA_est, x = vA_true, color = factor(format(mut_ratio, scientific = FALSE)))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
...
d_9 = read.csv("../combined_data/Set_9_output.csv") # Data for the standard set
d_9_std = d_9[d_9$all.gp==FALSE&d_9$Ne_exp!="1000_1000",]
d_12 = read.csv("../combined_data/Set_12_output.csv") # Data when ml_exp was varied
...
d_9 = read.csv("../combined_data/Set_9_output.csv") # Data for the standard set
d_9_std = d_9[d_9$all.gp==FALSE&d_9$Ne_exp!="1000_1000",]
d_15 = read.csv("../combined_data/Set_15_output.csv")
...
p_scale = ggplot(d_scale , aes(y=vA_est, x = vA_true, color = factor(scale))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
...
## Simulations with burnin (scale = 0.033) analysed with all.gp = FALSE (standard) or all.gp = TRUE (gametic phase LD not used)
...
p_all.gp = ggplot(d_all.gp, aes(y = vA_est, x = vA_true, color = all.gp)) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = TeX(r"($\tilde{\bf L}_0$)")) + 
  scale_color_manual(values = c("#D01C8B", "#0072B2"), labels = all.gp_labs) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) +
  theme(plot.title = element_text(hjust = 0.5))
...
## Simulations with burnin (scale = 0.033) analysed with Ne = nind = 1000
...
for(i in 1:length(Ne_type)){
  ...
  if(Ne_type_labs[i]==nind_exp){
    Ne_type_labs[i] = paste("Fixed:", Ne_type_labs[i])
  }else{
...
p_Ne = ggplot(d_9_Ne, aes(y = vA_est, x = vA_true, color = Ne_exp)) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = TeX(r"($N_E$)")) + 
  scale_color_manual(values = c("#D01C8B", "#0072B2"), labels = Ne_type_labs) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) +
  theme(plot.title = element_text(hjust = 0.5))
...
ggsave(p_Ne_res, width = 5, height = 5, file = "manuscript_figures/FigS3.jpg")
...
ggsave(p_b_0.033_combined, width = 12, height = 12, file = "manuscript_figures/Fig4.jpg")
...
ggsave(p_scale, width = 5, height = 5, file = "Fig.4D.jpg")
...
plot_lost_va = ggplot(d_va_lost, aes(y = ((va_true - va_left)/va_true), x = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_boxplot() +
  labs(y = TeX(r"(Fraction of the total $V_a(0)$ lost)"), x = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) 
...
d_va_lost[which((d_va_lost$va_true - d_va_lost$va_left) == max(d_va_lost$va_true - d_va_lost$va_left)),]$Set_ID
...
d_va_lost[which(d_va_lost$va_true - d_va_lost$va_left == min(d_va_lost$va_true - d_va_lost$va_left)),]$Set_ID
...
# Maximum loss
d_finescale_max = read.csv("../Set_7_c5_2024-12-19_01-46-27.974096_1.58565656565657e-07_0.5_2_1000_10_4_0_0_sim_1_finescale_va_ac3-n1_2025-03-19_17-59-21.412206.csv")
# Minimum loss
d_finescale_min = read.csv("../Set_9_bigbird_2024-12-26_00-57-27.25767_6.06545454545455e-08_0.5_2_1000_10_4_0_0_sim_1_finescale_va_ac3-n6_2025-03-19_18-33-04.254602.csv")
...
d_finescale$lost_va_type = ifelse(d_finescale$Set_ID=="Set_9_bigbird_2024-12-26_00-57-27.25767_6.06545454545455e-08_0.5_2_1000_10_4_0_0", "Min.", "Max.")

va_true_min = sum(d_finescale[d_finescale$Set_ID=="Set_9_bigbird_2024-12-26_00-57-27.25767_6.06545454545455e-08_0.5_2_1000_10_4_0_0",]$locuswise_va_true)
va_true_max = sum(d_finescale[d_finescale$Set_ID=="Set_7_c5_2024-12-19_01-46-27.974096_1.58565656565657e-07_0.5_2_1000_10_4_0_0",]$locuswise_va_true)
...
plot_finescale_va_lost = ggplot(d_finescale_sel, aes(y = locuswise_va_lost, x = abs(list_alpha), color = lost_va_type)) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"($V_a$ lost at a locus$)"), x = TeX(r"($|\eta|$)"), color = "Simulation type") +
  theme(text = element_text(size = 15)) +
  ...
  geom_line(aes(x = seq(0, 1.2, length = 28589), y = 0.5*(1/2000)*(1999/2000)*seq(0, 1.2, length = 28589)^2), linewidth = 0.1, color = "black")
...
d_scale = rbind(d_7[d_7$r==5e-07,], d_9_std, d_N1[d_N1$r==5e-07,])

d_scale$fract_va_lost = NA
d_scale$abs_va_lost = NA

for(set_id in d_scale$Set_ID){
  lost_va_data = d_va_lost[d_va_lost$Set_ID == set_id,]
  fract_va_lost = (lost_va_data$va_true - lost_va_data$va_left)/lost_va_data$va_true
  abs_va_lost = (lost_va_data$va_true - lost_va_data$va_left)
  d_scale[d_scale$Set_ID==set_id,]$fract_va_lost = fract_va_lost
  d_scale[d_scale$Set_ID==set_id,]$abs_va_lost = abs_va_lost
}
...
plot_lost_va_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = abs_va_lost, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
...
va_lost_combined_plot = plot_grid(plot_lost_va, plot_lost_va_bias, plot_finescale_va_lost, ncol = 2, nrow = 2, labels = "AUTO")

ggsave(va_lost_combined_plot, width = 10, height = 10, file = "manuscript_figures/FigS2.jpg")



plot_palpha_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = palpha_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73"))  +
  theme(legend.position = "bottom")
  
plot_balpha_slope_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = balpha_slope_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
...
plot_balpha_intercept_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = balpha_intercept_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
...
plot_palphaemp_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = palpha_emp, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
...
plot_palphaemp = ggplot(d_scale, aes(y = palpha_est,x = palpha_emp, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs( color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73"))  +
  theme(legend.position = "bottom") + geom_abline(slope = 1, intercept = 0)

plot_balpha_slopeemp = ggplot(d_scale, aes(y = balpha_slope_est,x = balpha_slope_emp, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs( color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73"))  +
  theme(legend.position = "bottom") + geom_abline(slope = 0.5, intercept = 0)
