...
title = ggdraw() + draw_label(paste("Map length in burnin = ", unique(d_5$r_msp*d_5$sequence_length), ", scale = ", d_5$scale, sep = ""), hjust = 0.5, size = 20)
...
p_b_2_0.045_ml = ggplot(d_N1 , aes(y=vA_est, x = vA_true, color = factor(sequence_length*r))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = "Map length (M)", title = paste("Map length in experiment = ", unique(d_N1$sequence_length*d_N1$r_exp), " M", sep = "")) + 
  scale_color_manual(values = c("#D01C8B", "#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) +
  theme(plot.title = element_text(hjust = 0.5))

ggsave(p_b_2_0.045_ml, width = 8, height = 8, file = "p_b_2_0.045_ml.jpg")
p_b_2_0.045_ml
...
p_scale_0.033 = ggplot(d_scale_0.033 , aes(y=vA_est, x = vA_true, color = factor(scale*shape))) +
  ...
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = TeX(r"($\mu_{\alpha}$)")) + 
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) +
  theme(plot.title = element_text(hjust = 0.5)) 
...
```{r}
p_b_0.033_combined = plot_grid(p_ml_exp_0.033, p_ben_0.033, p_scale_0.033, ncol = 2, labels = "AUTO")

title = ggdraw() + draw_label(paste("Map length in burnin = ", unique(d_9$r*d_9$sequence_length), sep = ""), hjust = 0.5, size = 20)
