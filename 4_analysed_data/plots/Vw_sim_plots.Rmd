...
p_b_0.045_ml = ggplot(d_N1 , aes(y=vA_est, x = vA_true, color = factor(sequence_length*r))) +
  ...
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = "Map length \nin Phase 1 (M)", title = TeX(r"($\eta_{scale}$ = 0.045)")) + 
...
ggsave(p_b_0.045_ml, width = 5, height = 5, file = "manuscript_figures/FigS1.jpg")
...
for(i in 1:length(Ne_type)){
  ...
  if(Ne_type_labs[i]==nind_exp){
    Ne_type_labs[i] = paste("Fixed:", Ne_type_labs[i])
      ...
      Ne_type_labs[i] = paste("Estimated:", Ne_type_labs[i])
      }
}
