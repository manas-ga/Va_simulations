...
p_scale = ggplot(d_scale , aes(y=vA_est, x = vA_true, color = factor(scale*shape))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
...
```{r}
p_b_0.033_combined = plot_grid(p_ml_exp_0.033, p_b_0.033_ml, p_ben_0.033, p_scale, p_all.gp, ncol = 2, labels = "AUTO")
...
plot_finescale_va_lost = ggplot(d_finescale_sel, aes(y = locuswise_va_lost, x = abs(list_alpha), color = lost_va_type)) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"($V_a$ lost at a locus$)"), x = TeX(r"(Non-neutral $\alpha$)"), color = "Simulation type") +
  theme(text = element_text(size = 15)) +
...
plot_lost_va_bias = ggplot(d_scale, aes(y = vA_est - vA_true,x = abs_va_lost, color = factor(round(shape*scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"),x = TeX(r"(Total $V_a$ lost)"), color = TeX(r"(Non-neutral $\mu_{\alpha}$)")) +
  theme(text = element_text(size = 15)) +
