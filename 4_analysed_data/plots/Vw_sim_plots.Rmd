...
ggsave(p_b_5_2_ml + labs(title = paste("scale = ", unique(d_7_ml$scale), sep = "")), width = 8, height = 8, file = "manuscript_figures/FigS1.jpg")
p_b_5_2_ml
...
ggsave(p_b_0.045_ml, width = 8, height = 8, file = "manuscript_figures/FigS2.jpg")
p_b_0.045_ml
...
```{r}
p_b_0.033_combined = plot_grid(p_ml_exp_0.033, p_b_0.045_ml, p_ben_0.033, p_scale_0.033,  ncol = 2, labels = "AUTO")
...
Plot the fraction of the initial additive genic variance lost by the end of the experiment as a function of non-neutral \mu_{\alpha}. Then, identify the simulation with the greatest loss and plot locuswise fraction V_a lost vs abs(\alpha) for this simulations. The target simulation is the sim 1 in the set "Set_7_c5_2024-12-18_22-42-43.451498_2.08686868686869e-08_0.5_2_1000_10_4_0_0"
```{r va_lost}

d_va_lost = read.csv("../combined_data/va_lost_combined.csv")

va_lost_plot = ggplot(d_va_lost, aes(y = ((va_true - va_left)/va_true), x = factor(shape*scale))) +
  theme_bw() +
  geom_boxplot() +
  labs(y = TeX(r"(Fraction of the initial $V_a$ lost)"), x = TeX(r"(Non-neutral $\mu_{\alpha}$)")) +
  theme(text = element_text(size = 15)) 

# Identify the worst simulation
