...
analyse_parents = function(c0,  
                           ...
                           calc_nR=TRUE, 
                           verbose=TRUE){ # Should there be at least one cross-over per meiosis?)
  ...
  return(list(L=L, nR=nR, svdL=if(LDdelta){list(UL=UL, DL=DL)}else{NULL}, seg_sites=seg_sites, seg_sites_neu=seg_sites_neu, seg_sites_ben=seg_sites_ben, mean_diversity=mean_diversity, va_true=va_true, vA_true=vA_true, vA_alpha_emp=vA_alpha_emp, parameters=alpha_properties[1:5]))
...
analyse_sim = function(Set_ID,                # The unique ID of the set of simulations that are controlled by a single R script
  ...
  sim_data = extract_slim_data$sim_params

  analysis_data = dara.frame("proj"=proj, "LDdelta"=LDdelta, "pa"=pa, "Vs"=Vs, "randomise"=randomise, "pdelta_method"=pdelta_method, "bdelta_method"=bdelta_method, "va_true"=parents_info$va_true, "vA_true"=parents_info$vA_true, "vA_est"=vA_est, vA_left, "vA_alpha_emp"=parents_info$vA_alpha_emp, "pdelta_emp"=parents_info$pdelta_emp, "bdelta_intercept_emp"=parents_info$bdelta_intercept_emp, "bdelta_slope_emp"=parents_info$bdelta_slope_emp, "sigma2delta_emp"=parents_info$sigma2delta_emp, "pdelta_est"=pdelta_est, "pdelta_var_est"=pdelta_var_est, "bdelta_intercept_est"=bdelta_intercept_est, "bdelta_slope_est"=bdelta_slope_est, "bdelta_var_est"=bdelta_var_est, "sigma2delta_est"=sigma2delta_est, "seg_sites"=parents_info$seg_sites, "seg_sites_neu"=parents_info$seg_sites_neu, "seg_sites_ben"=parents_info$seg_sites_ben, "seg_sites_del"=parents_info$seg_sites_del, "mean_diversity"=parents_info$mean_diversity)

  analysis_data = cbind(sim_data, analysis_data)

  write.table(analysis_data, file = paste(sim_param_path, "/", Set_ID, "_Data_analysis.csv", sep = ""),col.names = FALSE, row.names = FALSE, sep = ",")
