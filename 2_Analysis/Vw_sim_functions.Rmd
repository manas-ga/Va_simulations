...
analyse_parents = function(c_genome,  
                           ...
                           compute_svdL=FALSE,     # Should SVD of L be computed (overridden if LDdelta=TRUE)
                           LDdelta=FALSE,          # 
  ...
  return(list(L=L, Ltilde=Ltilde, nR=nR, svdL=if(compute_svdL | LDdelta){list(UL=UL, DL=DL)}else{NULL}, seg_sites=seg_sites, seg_sites_neu=seg_sites_neu, seg_sites_ben=seg_sites_ben, seg_sites_del=seg_sites_del, mean_diversity=mean_diversity, va_true=va_true, vA_true=vA_true, vA_alpha_emp=vA_alpha_emp, parameters=alpha_properties[1:5]))
