...
```
Fit model assuming no non-gametic phase disequilibrium

```{r fit_model_a, eval=run_model}

pointr::ptr("Ltilde", "parents_info$L")

m1<-Vw_model(c_genome = NULL,              # gametes genotypes 
             nR = parents_info$nR,         # matrix of non-recombinant probabilities between loci
             pbar0 = sim_data$pbar0,       # vector of allele frequencies at time-point 1
             ...
             Ltilde = Ltilde,    # list with elements UL and DL
...
and then again with `Ltilde` taking into account the gametic phase disequilibrium

```{r fit_model_b, eval=run_model}

m1_b<-Vw_model(c_genome = NULL,              # gametes genotypes 
             nR = parents_info$nR,         # matrix of non-recombinant probabilities between loci
             pbar0 = sim_data$pbar0,       # vector of allele frequencies at time-point 1
             pbar1 = sim_data$pbar1,       # vector of allele frequencies at time-point 1
             ngen1= sim_data$ngen1,    # number of generations between parents and time-point 1
