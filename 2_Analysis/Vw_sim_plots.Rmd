...
Needs datasets for set 9 and set 15
...
p_b_0.033_ml = ggplot(d_13 , aes(y=vA_est, x = if(alpha){vA_true_new}else{vA_true}, color = factor(sequence_length*r))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = "Map length \nin morgans \n(history)") + 
...
# However, limit only to simulations for which vA_true (or vA_true_new) is close to 0.075
d_scale1 = d_scale[if(alpha){which(0.074 <= d_scale$vA_true_new & d_scale$vA_true_new<= 0.076)}else{which(0.074 <= d_scale$vA_true & d_scale$vA_true_new <= 0.076)},]

if(alpha){
  max_loss_sim = d_scale1[which((d_scale1$va_true_new - d_scale1$va_left_new) == max(d_scale1$va_true_new - d_scale1$va_left_new)),]$Set_ID}else{
  max_loss_sim = d_scale1[which((d_scale1$va_true - d_scale1$va_left) == max(d_scale1$va_true - d_scale1$va_left)),]$Set_ID  
  }

message("Simulation with the geratest absolute loss in initial additive genic variance:")
max_loss_sim
message(paste("True V_A =", if(alpha){d_scale[d_scale$Set_ID==max_loss_sim,]$vA_true_new}else{d_scale[d_scale$Set_ID==max_loss_sim,]$vA_true}))
...
plot_palpha_bias = ggplot(d_scale, aes(y = vA_est - if(alpha){vA_true_new}else{vA_true},x = palpha_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
...
plot_balpha_slope_bias = ggplot(d_scale, aes(y = vA_est - if(alpha){vA_true_new}else{vA_true},x = balpha_slope_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
...
plot_balpha_intercept_bias = ggplot(d_scale, aes(y = vA_est - if(alpha){vA_true_new}else{vA_true},x = balpha_intercept_est, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
...
plot_palphaemp_bias = ggplot(d_scale, aes(y = vA_est - if(alpha){vA_true_new}else{vA_true},x = palpha_emp, color = factor(round(scale, digits = 4)))) +
  theme_bw() +
  geom_point() +
  labs(y = TeX(r"(Estimate of $V_A$ - True $V_A$)"), color = TeX(r"($\eta_{scale}$)")) +
  theme(text = element_text(size = 15)) +
