...
d_nb_poolseq_noQ = read.csv(file.path(data_path, "combined_data/Set_18_corrected_output.csv"))
...
p_scale = ggplot(d_scale , aes(y=vA_est, x = if(alpha){vA_true_new}else{vA_true}, color = factor(round(2*scale*shape, 2)))) +
  theme_bw() + 
  geom_point() + 
  geom_abline(intercept = 0, slope = 1) + 
...
pdf(file.path(output_path, "LD_eigenvector1.pdf"), width = 10, height = 10)
par(mfrow=c(2,2))
plot(abs(UL_0.5[,1]) ~ positions_0.5, xlab = "Position", ylab = "Absolute sitewise loadings", main = "0.5 morgan", cex.main = 2, cex.lab = 1.7, cex = 1)
plot(abs(UL_5[,1]) ~ positions_5, xlab = "Position", ylab = "Absolute sitewise loadings", main = "5 morgans", cex.main = 2, cex.lab = 1.7, cex = 1)
plot(abs(UL_50[,1]) ~ positions_50, xlab = "Position", ylab = "Absolute sitewise loadings", main = "50 morgans", cex.main = 2, cex.lab = 1.7, cex = 1)
plot(abs(UL_250[,1]) ~ positions_250, xlab = "Position", ylab = "Absolute sitewise loadings", main = "250 morgans", cex.main = 2, cex.lab = 1.7, cex = 1)
dev.off()
}
...
plot_lost_va = ggplot(d_scale, aes(y = if(alpha){((va_true_new - va_left_new)/va_true_new)}else{((va_true - va_left)/va_true)}, x = factor(round(2*shape*scale, digits = 2)), fill = factor(round(2*shape*scale, digits = 2)))) +
  theme_bw() +
  geom_boxplot() +
