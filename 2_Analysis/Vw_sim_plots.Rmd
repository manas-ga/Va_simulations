...
p_vA_va = ggplot(d_25, aes(y = vA_true_new, x = va_true_new, color = k)) +
 ...
 theme(legend.position = "bottom") +
  geom_abline(slope = 1, intercept = 0) +
  geom_smooth(method = "lm", se=FALSE) +
  labs(y = TeX(r"(True additive genetic variance ($V_A$))"), x = TeX(r"(True additive genic variance ($V_a$))"), color = "Dominance (|k|)", title = "Full simulations", subtitle = "(with dominance in history phase)") + 
...
p_pool_sites = ggplot(d_26, aes(y = vA_est, x = seg_sites, color = Type)) + 
  ...
  labs(x = "Number of segregating sites", y = TeX(r"(Estimate of $V_A$)"), color = "Pool-seq", title = "Simplified simulations", subtitle = "(0.5 morgan)") +
