...
```
### Simplified simulations analysed after simulated_pool_seq sampling of allele frequencies (Figure S5)
```{r simplified_polseq}
...
p_nb_poolseq = ggplot(d_nb_poolseq, aes(y = vA_est, x = vA_true_new, color = Type)) +
  ...
  scale_color_manual(values = c("#009E73", "#D01C8B", "#999999", "#0072B2", "#E59866")) + 
...
d_21 = read.csv(file.path(data_path, "combined_data/Set_21_corrected_output.csv"))
d_21 = add_new_va_data(new_va_data=d_new_va, target_data=d_21, verbose=TRUE)
d_21$k = gsub(".*_", x=unique(d_21$sim), "")

p_nb_dom = ggplot(d_21, aes(y = vA_est, x = vA_true_new, color = k)) +
  theme_bw() + geom_point() + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$)"), color = "Dominance") +
  theme(text = element_text(size = 15))+ 
  scale_color_manual(values = c("#0072B2", "#009E73", "#999999")) +
...
```
### Full simulations analysed after simulated_pool_seq sampling of allele frequencies (Figure S4)
