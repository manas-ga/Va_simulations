...
d_sanity$vA_BC_est = unlist(lapply(d_sanity$vA_BC, function(x){as.numeric(unlist(strsplit(x, "_", 2))[2])}))
...
p_sanity_bc = ggplot(d_sanity, aes(y = vA_BC_est, x = vA_true, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$ using B&C)"), color = "Map length \nin history") + 
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE)  
...
p_sanity_combined = plot_grid(p_sanity_us + theme(legend.position = "none"),
                              p_sanity_bc + theme(legend.position = "none"), nrow = 2)
p_sanity_combined = plot_grid(p_sanity_combined, legend_sanity, ncol = 2, rel_widths = c(1, 0.35))
