...
d_16$vA_BC_est_actual = unlist(lapply(d_16$vA_BC, function(x){as.numeric(unlist(strsplit(x, "_"))[3])}))
d_16$vA_BC_est_exact = unlist(lapply(d_16$vA_BC, function(x){as.numeric(unlist(strsplit(x, "_"))[7])}))
...
p_bc_simplified_bc_actual = ggplot(d_16, aes(y = vA_BC_est_actual, x = if(alpha){vA_true_new}else{vA_true}, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
...
p_bc_simplified_bc_exact = ggplot(d_16, aes(y = vA_BC_est_exact, x = if(alpha){vA_true_new}else{vA_true}, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
...
p_bc_simplified_combined = plot_grid(p_bc_simplified_bc_actual + theme(legend.position = "none"), 
                              p_bc_simplified_bc_exact + theme(legend.position = "none"), 
...
p_bc_simplified_combined = plot_grid(p_bc_simplified_combined, legend_16, ncol = 2, rel_widths = c(1, 0.15))

ggsave(p_bc_simplified_combined, file = file.path(output_path, "Fig5.jpg"), w = 9, h = 9)
