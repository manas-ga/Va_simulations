...
plot_lost_va_ml = ggplot(d_N1, aes(y = if(alpha){((va_true_new - va_left_new)/va_true_new)}else{((va_true - va_left)/va_true)}, x = factor(round(scale, digits = 4)), fill = factor(round(r*sequence_length, digits = 4)))) +
  theme_bw() +
  geom_boxplot() +
  labs(y = TeX(r"(Fraction of the total $V_a(0)$ lost)"), x = TeX(r"($\eta_{scale}$)"), fill  = TeX(r"($Map length$)")) +
  theme(text = element_text(size = 15)) +
...
p_ml_palpha_emp = ggplot(d_ml_history, aes(y = palpha_emp, x = factor(r*sequence_length))) +
  theme_bw() +
  geom_boxplot() +
  theme(text = element_text(size = 15)) +
...
p_ml_palpha_est = ggplot(d_ml_history, aes(y = palpha_est, x = factor(r*sequence_length))) +
  theme_bw() +
  geom_boxplot() +
  theme(text = element_text(size = 15)) +
  labs(x = "Map length in morgans (history)")
...
p_ml_balpha_slope_emp = ggplot(d_ml_history, aes(y = balpha_slope_emp, x = factor(r*sequence_length))) +
  theme_bw() +
  geom_boxplot() +
  theme(text = element_text(size = 15)) +
  labs(x = "Map length in morgans (history)")

p_ml_balpha_slope_emp

p_ml_balpha_slope_est = ggplot(d_ml_history, aes(y = balpha_slope_est, x = factor(r*sequence_length))) +
  theme_bw() +
  geom_boxplot() +
  theme(text = element_text(size = 15)) +
  labs(x = "Map length in morgans (history)")

p_ml_balpha_slope_est
...
d_ml_palpha_balpha = data.frame("palpha" = c(d_ml_history$palpha_emp, d_ml_history$palpha_est),
                       ...
                       "type" = c(rep("emp", nrow(d_ml_history)), rep("est", nrow(d_ml_history))),
...
p_sanity_us = ggplot(d_sanity, aes(y = vA_est, x = vA_true, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$ from Vw_model())"), color = "Map length in history") + 
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) + ylim(0, 0.35) + xlim(0, 0.13)

p_sanity_bc_actual = ggplot(d_sanity, aes(y = vA_BC_est_actual, x = vA_true, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$ using B&C (actual))"), color = "Map length in history") + 
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE)  + ylim(0, 0.35) + xlim(0, 0.13)

p_sanity_bc_exact = ggplot(d_sanity, aes(y = vA_BC_est_exact, x = vA_true, color = as.factor(sequence_length*r_msp))) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) + 
  labs(x = TeX(r"(True $V_A$)"), y = TeX(r"(Estimate of $V_A$ using B&C (exact))"), color = "Map length in history") + 
  scale_color_manual(values = c("#0072B2", "#999999", "#009E73")) + 
  theme(text = element_text(size = 15)) +
  geom_smooth(method = "lm", se=FALSE) + ylim(0, 0.35) + xlim(0, 0.13)
...
p_sanity_combined = plot_grid(p_sanity_bc_actual + theme(legend.position = "none"), 
                              ...
                              p_sanity_us + theme(legend.position = "none"), ncol = 3)
p_sanity_combined = plot_grid(p_sanity_combined, legend_sanity, nrow = 2, rel_heights = c(1, 0.35))

ggsave(p_sanity_combined, file = file.path(output_path, "sanity.jpg"), w = 12, h = 6)
