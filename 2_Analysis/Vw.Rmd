...
alpha_distribution<-function(alpha, p, tproj=NULL, logit=FALSE){
  ...
  logit_fit<-function(par, alpha, pmq, pq){
     bdelta_0<-par[1]
     pdelta<-par[2]
     -logLik(lm(alpha~pmq_trans(pmq, bdelta_0), weights=1/(pq^pdelta)))
  }

  if(logit){
    pdelta<-optim(c(1, 1), logit_fit, alpha=alpha, pmq=pmq, pq=pq)$par
    bdelta_0<-pdelta[1]
    pdelta<-pdelta[2]
    pred<-pmq_trans(pmq, bdelta_0)
    m1<-lm(alpha~pred-1, weights=1/(pq^pdelta))
  }else{
    pdelta<-optim(0, pdelta_fit, alpha=alpha, pmq=pmq, pq=pq, method="Brent", lower=-5, upper=5)$par
    pred<-pmq
    m1<-lm(alpha~pred, weights=1/(pq^pdelta))
  }  

  
  result<-list(pdelta=pdelta, 
                bdelta_0=if(logit){bdelta_0}else{coef(m1)["(Intercept)"]}, 
                bdelta_1=coef(m1)["pred"],
...
The function `plot_alpha_distribution` plots the $\alpha$'s against $p-q$ and overlays the expected $\alpha$ and its upper/lower 2.5\% quantiles given the parameters of a model fit. The argument `parameters` must be a list containing elements, `pdelta`, `bdelta_0`, `bdelta_1`, `sigma2delta` and logit. The output from `alpha_distribution` can be passed directly to the argument `parameters`. A second set of parameters (`parameters2`) can also be passed and the the expected $\alpha$ and upper/lower 2.5\% quantiles of this sceond fit are overlaid in blue.
...
plot_alpha_distribution<-function(alpha=NULL, p=NULL, tproj=NULL, parameters, parameters2=NULL, ...){
  ...
  pmq.x<-seq(-0.9999,0.9999, length=1000)
  ...
  if(!is.null(parameters2)){
    ...
    if(parameters2$logit){
      pred.x<-pmq_trans(pmq.x, parameters2$bdelta_0)
    }else{
      pred.x<-pmq.x
    }
    ...
    if(parameters2$logit){
      ...
      intercept<-parameters2$bdelta_0
    }
    slope<-parameters2$bdelta_1

    u.x<-qnorm(0.975, intercept+slope*pred.x, sd.x)
    m.x<-intercept+slope*pred.x
    l.x<-qnorm(0.025, intercept+slope*pred.x, sd.x)

    lines(m.x~pmq.x, col="blue")
    lines(u.x~pmq.x, col="blue", lty=2)
    lines(l.x~pmq.x, col="blue", lty=2)
  }
}
...
fit.model<-function(pdelta, bdelta, LDdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, tprojp, pbar0, pbar1, pbar2, nrep, N, Ne_factor=1, Vs, LLonly=FALSE, method = "REML", verbose=TRUE){
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(is.na(pdelta)){
    ...
    pdelta<-optim(0, fit.model, bdelta=bdelta, LDdelta = LDdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, tprojp=tprojp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, N=N, Ne_factor=Ne_factor, Vs=Vs, method = "L-BFGS-B", lower = -2, upper =2, control = list(fnscale=-1, factr = 1e+11), hessian=TRUE)
