...
fit.model<-function(pdelta, bdelta, LDdelta, nsnps, UL, DL, L, Ltilde=NULL, ngen2, ngen1, nind, tprojp, pbar0, pbar1, pbar2, nrep, N, Ne_factor=1, Vs, LLonly=FALSE, method = "REML", verbose=TRUE){
...
#    pointr::ptr("Ltilde", "L")
  ...
  pmq<-Ltilde%*%pmq
  int<-Ltilde%*%rep(1, nsnps)

  if(verbose){
    message("Computing SC...")
  }

  Ltilde<-t(tprojp)%*%Ltilde
 ...
 #   pointr::ptr("Ltilde", "L")
  ...
  if(proj=="LoM" | proj=="BLoM"){ 
    ...
    if(proj=="LoM"){
        ...
        sdLoM<-RSpectra::eigs(Ltilde*M, min(nind, nsnps))
    ...
    if(proj=="BLoM"){ 
        ...
        sdLoM<-RSpectra::eigs(t(UL)%*%(Ltilde*M)%*%UL, retain)
        retain<-sum(sqrt(sdLoM$values)>tol)
  ...
  if(is.na(pdelta)){
    ...
    pdelta<-optim(0, fit.model, bdelta=bdelta, LDdelta = LDdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, Ltilde=if(Ltilde.missing){NULL}else{Ltilde},  ngen2=ngen2, ngen1=ngen1, nind=nind, tprojp=tprojp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, N=N, Ne_factor=Ne_factor, Vs=Vs, method = "L-BFGS-B", lower = -2, upper =2, control = list(fnscale=-1, factr = 1e+11), hessian=TRUE)
  ...
  output<-fit.model(pdelta=pdelta, bdelta=bdelta, LDdelta = LDdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, Ltilde=if(Ltilde.missing){NULL}else{Ltilde}, ngen2=ngen2, ngen1=ngen1, nind=nind, tprojp=tprojp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=FALSE, N=N, Ne_factor=Ne_factor, Vs=Vs)
...
if(save){
  ...
  save(res, file=paste0(file.path(base_path, "2_Analysis/tmp"), "/res_", format(Sys.time(), "%H%M_%d_%m_%Y"), ".Rdata"))
}
