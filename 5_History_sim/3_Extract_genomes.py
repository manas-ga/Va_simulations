...
## Output
# A csv file containing the c matrix for genomes ("c_matrix.csv"). Note that the genomes in the SLiM output are such that genomes 2*i and 2*i - 1 belong to individual i
...
def extract_mutations_genomes(slim_output_path = "b_Interim_files/SLiM_Outputs/ERROR.txt",
                              mutations_path = "b_Interim_files/Mutations/ERROR.txt",
                              c_matrix_path = "b_Interim_files/C_Matrices/ERROR.csv",
                              sample_size = 1000):
    ...
    f_mutation = open(f"{mutations_path}/Mutations_{int(float(sys.argv[5]))}.txt", "w")
    ...
    numpy.savetxt(f"{c_matrix_path}/c_matrix_{int(sys.argv[5])}.csv", c_1, delimiter = ",")
