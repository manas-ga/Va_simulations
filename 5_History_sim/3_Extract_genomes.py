...
## Input variables:
# 1. The path of the text file containing the full output generated by the SLiM simulation (including filename)
# 2. The path where the text file containing mutations is to be stored (including filename)
# 3. The path where the .csv file containing the c matrix for haploid genomes is to be stored (including filename)
# 4. Size of the random sample to be drawn from the population (This feature is just there in case the full c matrix gets too large!)
...
## Output
# A text file of all mutations and their information
# A csv file containing the c matrix for genomes. Note that the genomes in the SLiM output are such that genomes 2*i and 2*i - 1 belong to individual i
...
def extract_mutations_genomes(slim_output_path, # Path of the .txt file containing the SLiM output
                              mutations_path,   # Path of the output .txt file containing mutations
                              c_matrix_path,    # Path of the output .csv file containing the c matrix for haploid genomes
                              sample_size):     # Number of individuals to be sampled randomly
    ...
    # If a genome does not contain any mutations, this generates an empty string, which then cannot be converted to int
    # Therefore, mutations that lack any mutations need to be treated separately

    for genome in range(0,len(genome_list)):
        genome_list[genome] = genome_list[genome][genome_list[genome].find("A")+2:].rstrip()

        if (len(genome_list[genome])>0): # If the genome contains at least one mutation
            
            # Convert the mutations in each genome into integers (currently they are strings)
            genome_list[genome] = genome_list[genome].split(" ")
    ...
    # Randomly sample individuals (sample_size is a parameter of the function)
    ...
    for genome in range(0,len(genome_list_sample)):
        ...
        # Need to ignore those genomes that lack any mutations
        if (len(genome_list_sample[genome])>0):
...
# Extract genomes based on command line arguments

extract_mutations_genomes(slim_output_path = sys.argv[1],
                              mutations_path = sys.argv[2],
                              c_matrix_path = sys.argv[3],
