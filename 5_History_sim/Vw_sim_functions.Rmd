...
extract_slim_data = function(Set_ID,                # The unique ID of the set of simulations that are controlled by a single R script
                             ...
                             n_ind_exp,             # Number of individuals in the experiment
                             n_sample=NULL,         # Number of individuals sampled from the parents' generation (useful if n_ind_exp is large)
                             n_cages,               # Number of replicates in the experiment
                             ngen_expt,             # Number of generations over which allele frequency changes are calculated
                             end_gen = 20000,       # The generation number of the parents' generation
                             randomise = TRUE,      # Optionally the reference allele can be randomised
                             delete_temp_files = TRUE){     
  
  
  # If n_sample is not provided extract the genomes of all individuals in the parents' genration
  ...
  # Read genomes
    ...
    # Delete the .txt file
    if(delete_temp_files){system(paste("rm", paste(mutations_path, "/", Set_ID, "_sim", sim, "_mutations_parents.txt", sep = ""), sep = " "))}
    ...
    for (cage in (1:n_cages)){
      ...
      for (gen in (end_gen+1):(end_gen+ngen_expt+1)){
        ...
        # Delete the .txt file
        if(delete_temp_files){system(paste("rm", paste(mutations_path, "/", Set_ID, "_sim", sim, "_cage", cage, "_mutations_", as.integer(gen), ".txt", sep = "")), sep = " ")}
  ...
  return(list(c0=c_ind/2, list_alpha=list_alpha, SNPs=SNPs, pbar1=pbar1, pbar2=pbar2))
...
analyse_parents = function(c0,  
                           ...
                           LDdelta=F,               # Whether L or diag(L) should be used to model vA using alpha distribution
  ...
  L = L = cov(c0)*(n_individuals-1)/(n_individuals) 
  
  message("Calculating nR..")
  ...
  return(list(L=L, nR=nR, seg_sites=seg_sites, seg_sites_neu=seg_sites_neu, seg_sites_ben=seg_sites_ben, va_true=vA_true, vA_true=vA_true, pdelta_emp=pdelta_emp, bdelta_intercept_emp=bdelta_intercept_emp, bdelta_slope_emp=bdelta_slope_emp))
