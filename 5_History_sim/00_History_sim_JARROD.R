...
if(!file.exists(paste(output_path, "/Data.csv", sep = ""))){
  col_names = as.matrix(t(c("Set_ID","Time","end_gen", "ngen_expt", "Ne", "n_ind_exp", "n_cages", "sequence_length", "r_msp", "r", "r_expt", "mu_msp", "mu", "mu_neutral", "shape", "scale", "mut_ratio", "proj", "LDdelta", "pa", "Vs", "randomise", "pdelta_method", "bdelta_method", "vA_true", "vA_est", "pdelta_est", "pdelta_var_est", "bdelta_intercept_est", "bdelta_slope_est", "seg_sites")))
...
r = 1.4e-06                            # Recombination rate (per site per generation) during the forward simulation of history
r_expt = 1.4e-06                       # Recombination rate to be used during during the experiment (Drosophila melanogaster ~ 1.4e-08)
r_msp = 1.4e-08                        # Recombination rate for the initial msprime simulation
...
mu_list = seq(1.3e-07, 1.3e-06, length = nsims)  # If mu is to be varied in order to vary true Vw # (1.5-6.0 e-06 works splendidly)
...
scale_list = seq(0.025, 0.025, length = nsims)  # Vector of Scale of the gamma DFE
...
pdelta_method = "optim" # "optim" or "manual" or "fixed". If "this is anything else"no_analysis", the estimate of Vw is not calculated, but the rest of the code still runs.
...
for (sim in 1:nsims){
  ...
  if(analyse){
      ...
      ### Fit the model serially varying pdelta, select the estimate that has the highest log likelihood
          ...
          LL<-vA_est_temp<-bdelta_intercept_temp<-bdelta_slope_temp<-1:nseq # Creating empty vectors to store pdelta and estimated vA in each model
  ...
  #rm(list = c("L", "L_ret", "c_genome", "c_ind", "c_ind_ret", "NRF")) # Remove memory-heavy objects before the next simulation starts 
