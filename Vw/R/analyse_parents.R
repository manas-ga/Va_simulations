...
analyse_parents = function(c_genome,  
  ...
  a = sum(1/(1:(nrow(c_genome)-1)))
  theta = ncol(c_genome)/(a*HapLength)
  ...
  return(list(L=L, Ltilde=Ltilde, nR=nR, svdL=if(compute_svdL | LDalpha){list(UL=UL, DL=DL)}else{NULL}, seg_sites=seg_sites, seg_sites_neu=seg_sites_neu, seg_sites_ben=seg_sites_ben, seg_sites_del=seg_sites_del, mean_diversity=mean_diversity, theta=theta, va_true=va_true, vA_true=vA_true, vA_alpha_emp=vA_alpha_emp, parameters=alpha_properties[1:5]))
