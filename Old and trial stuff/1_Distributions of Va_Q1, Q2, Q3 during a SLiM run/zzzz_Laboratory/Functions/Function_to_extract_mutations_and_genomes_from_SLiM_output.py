...
# Builds a c matrix, whose rows are genomes and columns are sites. If a mutation is present in a genome, the corresponding element of the c matrix is 1, otherwise it is 0.
...
def extract_mutations_genomes(slim_output_path = "1_Interim_files/SLiM_Outputs/Output_101000.txt", mutations_path = "1_Interim_files/Mutations/mutations.txt", all_genomes_path = "1_Interim_files/Genomes/all_genomes.txt", c_matrix_path = "1_Interim_files/C_Matrices/c_matrix.csv"):
    ...
    # Create an empty list to store genomes from subpopulation p2 (i.e. the sampled genomes). Later this list will be edited to remove unnecessary text and only have the list of mutations for each genome.
    a = []
