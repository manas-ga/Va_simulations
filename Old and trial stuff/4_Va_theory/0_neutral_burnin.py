...
demog_model.add_population(initial_size=3000)
...
for m in ots.mutations():
  ...
  for sid, md in zip(slim_ids, md_list):
    ...
    if sid not in mut_map:
      ...
      # DFE is gamma
      #mut_map[sid] = freq_array[numpy.random.randint(0, len(freq_array))]*(min(numpy.random.gamma(shape = 0.2, scale = float(sys.argv[1])), 1.0)) # The max() function prevents negative fitnesses
      #mut_map[sid] = freq_array[numpy.random.randint(0, len(freq_array))]*(min(numpy.random.gamma(shape = 0.2, scale = 0.15), 1.0)) # The max() function prevents negative fitnesses
...
ots.dump("C:/Academics/Post-doc/Va_simulations/4_Va_theory/b_Interim_files/Msprime_outputs/neutral_burnin.trees")
