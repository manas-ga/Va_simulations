...
EVw<-seq(1e-6, 0.1, length=nsim)    # Expected genetic variance for fitness
...
fit.model<-function(pdelta, bdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, projp, pbar0, pbar1, pbar2, nrep, nR, Ne_factor=1, LLonly=FALSE){
  ...
  if(method=="REML"){
    if(is.na(bdelta[1]) & is.na(bdelta[2])){
        m1<-asreml(delta~pmq, random = ~vm(locus, SC), data=dat.gaussian, Cfixed=TRUE)
    }
    if(is.na(bdelta[1]) & !is.na(bdelta[2])){
        m1<-asreml(delta~offset(pmq), random = ~vm(locus, SC), data=dat.gaussian, Cfixed=TRUE)
    }
    if(!is.na(bdelta[1]) & is.na(bdelta[2])){
        m1<-asreml(I(delta-bdelta[1])~pmq-1, random = ~vm(locus, SC), data=dat.gaussian, Cfixed=TRUE)
    }
    if(!is.na(bdelta[1]) & !is.na(bdelta[2])){
      warning("asreml doesn't allow models without fixed effects, so intercept fitted but replaced with bdelta[1]!")
        m1<-asreml(I(delta-bdelta[1])~offset(pmq), random = ~vm(locus, SC), data=dat.gaussian, Cfixed=TRUE)
    } 
  }
