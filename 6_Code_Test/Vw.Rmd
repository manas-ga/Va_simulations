...
alpha_distribution<-function(alpha, p, tproj=NULL, plot=FALSE, ...){
  ...
  if(plot){
    ...
    abline(result$bdelta_int, result$bdelta_slope)

    pmq.x<-seq(-0.9999,0.9999, length=100)
    pq.x = (pmq.x+1)*(1-pmq.x)/8

    sd.x<-sqrt(result$sigma2delta*pq.x^result$pdelta)
    
    lines(qnorm(0.975, result$bdelta_int+result$bdelta_slope*pmq.x, sd.x)~pmq.x)
    lines(qnorm(0.025, result$bdelta_int+result$bdelta_slope*pmq.x, sd.x)~pmq.x)
  }  
...
alpha_distribution(alpha, p, plot=TRUE)
# Good
...
fit.model<-function(pdelta, bdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, tprojp, pbar0, pbar1, pbar2, nrep, N, Ne_factor=1, Vs, LLonly=FALSE){
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(is.na(pdelta)){
    ...
    pdelta<-optim(0, fit.model, bdelta=bdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, tprojp=tprojp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, N=N, Ne_factor=Ne_factor, Vs=Vs, method = "L-BFGS-B", lower = -2, upper =2, control = list(fnscale=-1), hessian=TRUE)
  ...
  output<-fit.model(pdelta=pdelta, bdelta=bdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, tprojp=tprojp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=FALSE, N=N, Ne_factor=Ne_factor, Vs=Vs)
