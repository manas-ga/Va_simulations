...
fit.model<-function(pdelta, bdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, projp, pbar0, pbar1, pbar2, nrep, nR, Ne_factor=1, LLonly=FALSE){
  ...
  if(method=="REML"){
    if(is.na(bdelta[1]) & is.na(bdelta[2])){
        m1<-asreml(delta~pmq, random = ~vm(locus, SC, singG="PSD"), data=dat.gaussian, Cfixed=TRUE)
    }
    if(is.na(bdelta[1]) & !is.na(bdelta[2])){
        m1<-asreml(delta~offset(pmq), random = ~vm(locus, SC, singG="PSD"), data=dat.gaussian, Cfixed=TRUE)
    }
    if(!is.na(bdelta[1]) & is.na(bdelta[2])){
        m1<-asreml(I(delta-bdelta[1])~pmq-1, random = ~vm(locus, SC, singG="PSD"), data=dat.gaussian, Cfixed=TRUE)
    }
    if(!is.na(bdelta[1]) & !is.na(bdelta[2])){
      warning("asreml doesn't allow models without fixed effects, so intercept fitted but replaced with bdelta[1]!")
        m1<-asreml(I(delta-bdelta[1])~offset(pmq), random = ~vm(locus, SC, singG="PSD"), data=dat.gaussian, Cfixed=TRUE)
    } 
  }
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(is.na(pdelta)){
    ...
    pdelta<-optim(0, fit.model, bdelta=bdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, projp=projp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, nR=nR, Ne_factor=Ne_factor, method = "L-BFGS-B", lower = -5, upper =5, control = list(fnscale=-1), hessian=TRUE)
  ...
  output<-fit.model(pdelta=pdelta, bdelta=bdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, projp=projp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=FALSE, nR=nR, Ne_factor=Ne_factor)
  ...
  if(method=="REML"){
    ...
    if(is.na(bdelta[1]) & is.na(bdelta[2])){
        bdelta<-summary(output$model, coef=TRUE)$coef.fixed[c("(Intercept)", "pmq"),1]
        S<-output$model$Cfixed[c("(Intercept)", "pmq"),c("(Intercept)", "pmq")]
    }
    if(is.na(bdelta[1]) & !is.na(bdelta[2])){
       bdelta[1]<-summary(output$model, coef=TRUE)$coef.fixed["(Intercept)",1]
  ...
  if(method=="MCMC"){
    ...
    bdelta<-colMeans(output$model$Sol[,c("(Intercept)", "pmq")])
  }
  ...
  return(list(Vw_est=Vw_est, data=output$data, model=output$model, SC=output$SC, pdelta=output$pdelta, bdelta=bdelta, pdelta_var=pdelta_var, bdelta_var=S))
