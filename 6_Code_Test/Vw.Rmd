...
fit.model<-function(pdelta, bdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, projp, pbar0, pbar1, pbar2, nrep, N, Ne_factor=1, LLonly=FALSE){
  ...
  if(Vs=="LoNL"){
     ...
     message("Computing covp over ngen2 - ngen1 genrations...")
     covp<-(L*N)%*%covp%*%(L*N)
     pmq<-(L*N)%*%pmq
     int<-(L*N)%*%rep(1, nsnps)
  }
  ...
  int_proj<-t(matrix(int, nsnps,nrep))%*%projp
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(proj=="LoM" | proj=="BLoM"){ 
     ...
     for (x in (ngen1 + 2):(1+ngen2-ngen1)){
     ...
     for (y in (ngen1 + 2):(1+ngen2-ngen1)){
  ...
  if(is.na(pdelta)){
    ...
    pdelta<-optim(0, fit.model, bdelta=bdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, projp=projp, pbar0=pbar0, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, nR=nR, Ne_factor=Ne_factor, method = "L-BFGS-B", lower = -2, upper =2, control = list(fnscale=-1), hessian=TRUE)
