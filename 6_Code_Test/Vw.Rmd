...
EVw<-seq(0, 0.14, length=nsim)    # Expected genetic variance for fitness 
```
...
Vw_sim<-function(nind,                   # Number of parental flies
  ...
  MSP_SimulatedBasePopulation<-simulate_base_population(Nflies=nind,HapLength=HapLength,RecombRate=RecombRate.msprime, MutationRate=MutationRate.msprime,Ne=Ne, Fstem=Fstem, randomise_state_labels=TRUE, msp_path=msp_path)
  ...
  for(j in 1:nrep){
    ...
    Forward_SimulatedExperimentalPopulation<-simulate_forward_population(NGen=ngen,fitness_effects=a,HapLength=HapLength,starting_haps=MSP_SimulatedBasePopulation$Parental,vars=MSP_SimulatedBasePopulation$SNPs,RecombRate=RecombRate.experiment, AtleastOneRecomb=AtleastOneRecomb, Fstem="~/Work/Vw/Data/Intermediate/G",sdwe=sqrt(EVe))
...
fit.model<-function(pdelta, nsnps, UL, DL, L, ngen2, ngen1, nind, projp, pbar1, pbar2, nrep, LLonly=FALSE){
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(is.null(pdelta)){
    ...
    pdelta<-optim(0, fit.model, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, projp=projp, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=TRUE, control = list(fnscale=-1))$par

  }

  output<-fit.model(pdelta=pdelta, nsnps=nsnps, UL=UL, DL=DL, L=L, ngen2=ngen2, ngen1=ngen1, nind=nind, projp=projp, pbar1=pbar1, pbar2=pbar2, nrep=nrep, LLonly=FALSE)

  if(LDdelta){
    TrV<-sum(diag(DL)^(2*(output$pdelta+1)))
  }else{
    TrV<-sum(diag(L%*%diag(diag(L)^output$pdelta)))
  }

  if(method=="REML"){
    Vw_est<-summary(output$model)$varcomp[1,1]*TrV
  }
  if(method=="MCMC"){
    Vw_est<-posterior.mode(output$model$VCV[,1])*TrV
  }
