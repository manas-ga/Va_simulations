...
library(MCMCglmm)
library(asreml)
source("~/Work/Vw/R/Recombination_from_msprime.R")
#rmarkdown::render("~/Work/Vw/R/Vw.Rmd")
```
...
```{r Genome_and_Experiment_Parameters} 
###########################################
#          Pop Gen Parameters             #
...
RecombRate<-1.4e-8      
AtleastOneRecomb<-TRUE
MutationRate=1.3e-9
Ne=1e6

nsim<-1            # number of simulations

HapLength<-as.integer(seq(20000,20000,length=nsim))   # haplotype length
...
if(Sys.info()["nodename"]=="basden.bio.ed.ac.uk"){  
  msp_path="~/miniconda2/bin/"
}else{
  msp_path=""
}
...
It is also possible to switch the sign of the allelic effect so minor alleles are selected against when `minor_d=TRUE`.
...
Vw_sim<-function(nind,                   # Number of parental flies
                 ...
                 AtleastOneRecomb=TRUE,  # Should there be a minimum of one recombination event per meiosis?
                 ...
                 Fstem="~/Work/Vw/Data/Intermediate/G",  # File path for storing msprime files
                 msp_path                # File path to msprime executable
                 ){

  MSP_SimulatedBasePopulation<-simulate_base_population(Nflies=nind,HapLength=HapLength,RecombRate=RecombRate, MutationRate=MutationRate,Ne=Ne, Fstem=Fstem, randomise_state_labels=TRUE, msp_path=msp_path)
  # simulate genomes of parental population 

  nsnps<-length(MSP_SimulatedBasePopulation$SNPs)
  # number of polymorphic loci in parental population

  C0<-(MSP_SimulatedBasePopulation$Parental[1:nind,,drop=FALSE]+MSP_SimulatedBasePopulation$Parental[nind+1:nind,,drop=FALSE])/2
  ...
  for(j in 1:nrep){
    ...
    Forward_SimulatedExperimentalPopulation<-simulate_forward_population(NGen=ngen,fitness_effects=a,HapLength=HapLength,starting_haps=MSP_SimulatedBasePopulation$Parental,vars=MSP_SimulatedBasePopulation$SNPs,RecombRate=RecombRate, AtleastOneRecomb=AtleastOneRecomb, Fstem="~/Work/Vw/Data/Intermediate/G",sdwe=sqrt(EVe))
  ...
  return(list(C0=C0, pbar1=pbar1, pbar2=pbar2, SNPs=MSP_SimulatedBasePopulation$SNPs, Vw0=Vw0, Vw2=Vw2, L=if(save.L){L}else{NULL}, svdL=if(save.svdL){list(UL=UL, DL=svdC$d[1:retain])}else{NULL}))
...
The variance in allele frequency change *due to selection*, $VAR[E[\Delta {\bf p}]]$, is assumed to be proportional to $(n_g{\bf L}){\bf L}^{p_{\Delta}}({\bf L}n_g)$ (`Vs="L"`) or $({\bf L}\circ {\bf N}){\bf L}^{p_{\Delta}}({\bf L}\circ {\bf N})$ (`Vs="LoNL"`) where $n_g$ is the number of generations and $p_{\Delta}$  can be specified (`pdelta`). Theoretically, we expect allele frequency change *due to selection* to be proportional to $({\bf L}\circ {\bf N}){\bf L}^{p_{\Delta}}({\bf L}\circ {\bf N})$ where $p_{\Delta}=p_{\alpha}$ although `Vs="L"` with $p_{\Delta}=p_{\alpha}$ might serve as a good approximation. It is also possible to ignore LD when defining ${\bf L}^{p_{\Delta}}$ above using `LDdelta=FALSE`.
...
When projected allele frequencies are analysed we need to derive the covariance structure for the projected allele frequency change due to selection from how we have chosen to model the covariance structure of unprojected allele frequency changes specified through $p_\Delta$. With projection we have
...
form_nR<-function(SNPs,                   # SNP positions
                  RecombRate,             # Recombination rate between adjacent sites
                  HapLength,              # Genome size
                  AtleastOneRecomb=TRUE   # Should there be at least one cross-over per meiosis?
                  ){
  ...
  if(AtleastOneRecomb){
    ...
    # could probably do this more efficiently using Haldane's mapping function
    qfun<-qpois
    dfun<-dpois
    pfun<-ppois
  }  

  maxn<-qfun(1-1e-5, RecombRate*HapLength)
...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  attr(invSC,"rowNames")<-as.character(1:retain) # Used for sparse-form matrices
  attr(invSC,"colNames")<-as.character(1:retain) # Used for sparse-form matrices
  dimnames(invSC) <- list(1:retain,1:retain)  # used for full-form matrices
  ...
  if(method=="REML"){
    m1<-asreml(delta~1, random = ~vm(locus, invSC), data=dat.gaussian)
  }
  if(method=="MCMC"){
  ...
  return(list(Vw_est=Vw_est, data=dat.gaussian, model=m1, invSC=invSC))
...
```{r run_simulations, results="hide", eval=FALSE} 

Vw0<-Vw2<-Vw_est<-1:nsim
...
for(i in 1:nsim){
  ...
  sim<-Vw_sim(nind=nind,                 
              ...
              save.L=TRUE,
              save.svdL=TRUE, 
              Fstem="~/Work/Vw/Data/Intermediate/G",
              msp_path=msp_path)
...
if(save){
  res<-list(HapLength=HapLength, RecombRate=RecombRate, AtleastOneRecomb=AtleastOneRecomb, 
  MutationRate=MutationRate, Ne=Ne, nind=nind, nrep=nrep, ngen=ngen, EVe=EVe, EVw=EVw, proj=proj, pdelta=pdelta, pa=pa, palpha=palpha, LDalpha=LDalpha, LDdelta=LDdelta, method=method, Vw0=Vw0, Vw2=Vw2, Vw_est=Vw_est)
  save(res, file=paste0("~/Work/Vw/Data/Intermediate/res_", format(Sys.time(), "%H%M_%d_%m_%Y"), ".Rdata"))
}
...
d_data<-read.table("~/Work/Vw/Data/Raw/mu1.3e-9__r1.4e-8_2000Haplotypes.tsv", header=TRUE)
