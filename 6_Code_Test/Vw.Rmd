...
Vw_model<-function(C0,          # parental genotypes (rows individuals, columns loci, coded as 0, 1/2 or 1) 
  ...
  if(method=="REML"){
    ...
    if(!is.na(bdelta[1]) & is.na(bdelta[2])){
        bdelta[2]<-summary(output$model, coef=TRUE)$coef.fixed[,1]
  ...
  if(LDdelta){
    ...
    aLa<-sum(diag(L)*pred^2)-sum(diag(t(X)%*%diag(diag(L))%*%X%*%S))
  }

  Vw_est<-TrV+aLa

  return(list(Vw_est=Vw_est, data=output$data, model=output$model, SC=output$SC, pdelta=output$pdelta, bdelta=bdelta, pdelta_var=pdelta_var))
