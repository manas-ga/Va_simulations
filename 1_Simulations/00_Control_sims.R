...
slim_history_path = file.path(base_path, "1_History_dominance.slim")                                                    ## SLiM script running the history
...
slim_expt_path = file.path(base_path, "4_Experiment_dominance.slim")                                                    ## SLiM script running the experiment
...
output_freq = 50                       # The frequency with which summary stats are to be recorded in the history phase 
...
seed = sample(1:100000000, size = n_cages + 1)    # random seed for the history phase + replicates
message(paste("Random seed history phase = ", seed[1], sep = ""))
message("Random seeds experiment phase:")
seed[-1]
...
for (sim in 1:nsims){
      ...
      system(paste("python", msprime_burnin_path, Ne, n_ind, sequence_length, r_msp, mu_msp, shape, scale, slim_output_path, mut_ratio, DFE, mean_alpha, sqrt(var_alpha), Set_ID, sim, seed[1]))
      ...
      system(paste("python", msprime_add_neutral_path, slim_output_path, neutral_sites, Set_ID, sim, seed[1]))
      ...
      flip_arg12 = paste("-d seed=", seed[1], sep = "")
      ...
      for (cage in (1:n_cages)){
        ...
        expt_arg20 = paste("-d seed=", seed[1+cage], sep = "")
      ...
      # Same dominance related information in the "sim" column
      
      if(use_dominance){
        sim = paste(sim, "_", "k=", k, sep = "")
      }
      ...
      message(paste("Simulation", sim, "of set", Set_ID, "completed at", Sys.time())) 
